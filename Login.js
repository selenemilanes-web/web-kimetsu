$("#login").on("click", function () {
    ...
    $.ajax({
        method: "POST",
        url: "login.php",
        data: {"nom": $("#userlogin").val(), "contrasenya": $("#passwordlogin").val()}, //obtiene el valor del input #nom y #contrasenya del html
        ...
        success: function (data) { //En "data" nos está cogiendo el "json_encode" del PHP.
            if (data["Estat"] == "OK"){
                console.log(data);
                $('#registeredusers').html("Bienvenido"+JSON.stringify(data["User"]));
                ...
                document.forms[0].reset(); //to clear the form "Iniciar Sesión" for the next entries


            }if (data["Estat"] == "KO") {
                console.log(data);
                $('#registeredusers').html(JSON.stringify(data["Error"]+" "+data["User"]));
